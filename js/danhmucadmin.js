$(document).ready(function() {
            ...
            success: function(datarp) {
                if (datarp['add_messages'] == 'successfull') {
                    array_id.forEach(function(element) {
                        var thongbao = "Delete thành công sản phẩm " + element;
                        addthongbao(thongbao)
                    })
    ...
    $('.ad_dms_right_ttc_option_xoavungchon').on('click', function() {
            if (confirm("Bạn có chắc muốn xóa những mục đã chọn")) {
                ...
                array_checkbox_xoa.each(function(element) {
                    if ($(this).is(":checked")) {
                        array_id.push($(this).data('madmxoa'))
                    }
                })
    ...
    $(document).on('click', '.ad_dms_content_table_thaotac_edit', function() {
        ...
        $.ajax({
            ...
            dataType: "json",
            success: function(datarp) {
                $('.dm_ad_edit_content_madm').html(datarp['MaDM'])
                $('.dm_ad_edit_tendm').val(datarp['TenDM'])
                $('.dm_ad_edit_content_hadm').attr('src', './upload/image/danhmuc/' + datarp['HADM'] + '')
            }
        })
    })
    $('.dm_ad_edit_top_save').on("click", function() {
        ...
        if (FileUploadPath == '') {
            var tendm = $('.dm_ad_edit_tendm').val()
            var madm = $('.dm_ad_edit_content_madm').html()
            $.ajax({
                url: "./ajax_danhmucadmin.php",
                method: "post",
                data: {
    ...
    $(document).on('submit', '.form_hadm', function(e) {
        e.preventDefault();
        var tendm = $('.dm_ad_them_tendm').val()
        var dulieu = new FormData(this)
        dulieu.append('action', 'add')
        dulieu.append('tendm', tendm)
        $.ajax({
            url: "./ajax_danhmucadmin.php",
            method: "post",
